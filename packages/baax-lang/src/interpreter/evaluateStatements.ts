...
export function evaluateTopLevelStatements(
  ...
  for (const statement of ast.statements) {
    switch (statement.kind) {
      case "ImportStatement": {
        const module = system.module.read(statement.src.value);
        // TODO: if a class, define value and type

        context.valueScope.define(statement.identifier.value, module);
        break;
      }

      default:
        evaluateStatement(statement, context, system);
    }
  }
}
...
export function evaluateStatement(
  ...
  switch (statement.kind) {
    ...
    case "NamedRecordDefinitionStatement": {
      ...
      if (context.types.has(statement.namedRecordDefinition.identifier.value)) {
        console.warn(
    ...
    case "ConstantDefinition": {
      ...
      const result = evaluateExpression(statement.expression, context, system);
    ...
    case "MatchFunction": {
      ...
      context.valueScope.define(statement.identifier.value, matchFuncInstance);
      break;
    }
